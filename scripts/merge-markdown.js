...
function transformFrontmatter(content) {
    ...
    return content.replace(/^---\s*\n([\s\S]*?)\n---/m, (match, frontmatter) => {
        ...
        const sourcebook = frontmatter.match(/sourcebook:\s*['"]?(.*?)['"]?\s*$/m)?.[1] || 'Unknown';
...
/**
 * Compares filenames for sorting
 * @param {string} a
 * @param {string} b
 * @returns {number}
 */
function compareFilenames(a, b) {
    ...
    const [fileA, fileB] = [a, b].map(f => path.basename(f));
    const [numA, numB] = [fileA, fileB].map(f => 
        parseInt(f.match(/^\d+/)?.[0] || '0')
    );
    
    return numA !== numB ? numA - numB : fileA.localeCompare(fileB);
}
